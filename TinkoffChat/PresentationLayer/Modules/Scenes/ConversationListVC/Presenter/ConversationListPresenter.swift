...
protocol IConversationListPresenter: CommunicationManagerDelegate {
    var view: IConversationListView? { get set }
    func setupFRC()
    func getlastMessageFromConversationsWith(_ id: String) -> Message?
    func attachView(_ view: IConversationListView)
}
...
class ConversationListPresenter: IConversationListPresenter {
    ...
    func setupFRC() {
        let frc = frcService.configureAndGetFRC(with: frService.getAllConversations(), sectionKey: "user.isOnline")
        DispatchQueue.main.async {
            self.view?.setupView(with: frc)
        }
    }
